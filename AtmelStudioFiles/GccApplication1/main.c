...
int main (void)
{
	...
	while(1) {
		...
		*/
		/*
		char* test1 = (char*) malloc (sizeof(char) * 20);
		char* test2 = (char*) malloc (sizeof(char) * 20);
		char* test3 = (char*) malloc (sizeof(char) * 20);
		char* test4 = (char*) malloc (sizeof(char) * 20);
		char* test5 = (char*) malloc (sizeof(char) * 20);
		char* test6 = (char*) malloc (sizeof(char) * 20);
		char* test7 = (char*) malloc (sizeof(char) * 20);
		char* test8 = (char*) malloc (sizeof(char) * 20);
		char* test9 = (char*) malloc (sizeof(char) * 20);
		char* test10 = (char*) malloc (sizeof(char) * 20);
		char* test11 = (char*) malloc (sizeof(char) * 20);
		char* test12 = (char*) malloc (sizeof(char) * 20);
		char* test13 = (char*) malloc (sizeof(char) * 20);
		char* test14 = (char*) malloc (sizeof(char) * 20);
		char* test15 = (char*) malloc (sizeof(char) * 20);
		char* test16 = (char*) malloc (sizeof(char) * 20);
		char* test17 = (char*) malloc (sizeof(char) * 20);
		char* test18 = (char*) malloc (sizeof(char) * 20);
		char* test19 = (char*) malloc (sizeof(char) * 20);
		char* test20 = (char*) malloc (sizeof(char) * 20);	
		char* test21 = (char*) malloc (sizeof(char) * 20);
		
		for (uint8_t i = 0; i < 5; i++) {
			sprintf(test1, "hello %i", 100);
			sprintf(test2, "hello %i", 100);
			sprintf(test3, "hello %i", 100);
			sprintf(test4, "hello %i", 100);
			sprintf(test5, "hello %i", 100);
			sprintf(test6, "hello %i", 100);
			sprintf(test7, "hello %i", 100);
			sprintf(test8, "hello %i", 100);
			sprintf(test9, "hello %i", 100);
			sprintf(test10, "hello %i", 100);
			sprintf(test11, "hello %i", 100);
			sprintf(test12, "hello %i", 100);
			sprintf(test13, "hello %i", 100);
			sprintf(test14, "hello %i", 100);
			sprintf(test15, "hello %i", 100);
			sprintf(test16, "hello %i", 100);
			sprintf(test17, "hello %i", 100);
			sprintf(test18, "hello %i", 100);
			sprintf(test19, "hello %i", 100);
			sprintf(test20, "hello %i", 100);
			sprintf(test21, "hello %i", 100);
		}
		*/
		...
		PORTD ^= (1<<7);
			...
			u8g2_DrawStr(&u8g2, 0, 10, "Saturday");// Day of the week
			u8g2_SetFont(&u8g2, u8g2_font_6x13_tr );
			u8g2_SetFontMode(&u8g2, 2);
			u8g2_DrawStr(&u8g2, 0, 50, "Temp:");// Temperature Reading
			u8g2_DrawStr(&u8g2, 0, 65, "32.123");// Temperature Reading
			u8g2_DrawStr(&u8g2, 40, 65, "c");// Temperature Measurement
			u8g2_DrawStr(&u8g2, 0, 85, "Humidity:");// Temperature Reading
			u8g2_DrawStr(&u8g2, 0, 100, "312" );// Humidity Reading
			u8g2_DrawStr(&u8g2, 40, 100, "  %");// Humidity Measurement
			u8g2_DrawStr(&u8g2, 21, 28, fore[4].status);// Humidity Measurement
			u8g2_SetFont(&u8g2, u8g2_font_unifont_t_76);
			u8g2_DrawGlyph(&u8g2, 0, 30, 0x2613);
			u8g2_SendBuffer(&u8g2);
		_delay_ms(1000);
		/*
		free(test1);
...
void mode_A(struct Reading *reading) {
	...
	// Handles leading 0 on hour
	if (hTen) {
		snprintf(clockReading,15,"%i%i:%i%i:%i%i",  hTen, hOne, mTen, mOne, sTen, sOne);
	} else {
...
void mode_C(struct Reading reading) {
	...
	if (reading.celsius) {
		...
		snprintf(unit1, 10,  " %c%c", 0xB0, 'C');
	} else {
		int fahrenheit = (reading.temperature * 9/5) + 32;
		snprintf(str1,20, "Temperature:  %i", fahrenheit);
		snprintf(unit1,10, " %c%c", 0xB0, 'F');
	}
